...
public class AlertDaemon extends Thread {
    ...
    public AlertDaemon() {
        ...
        Runtime.getRuntime().addShutdownHook(
                new Thread(() -> {
                    ...
                    System.out.println("AlterDaemon Thread 종료 중");
                })
        );
    }
    ...
    public void run() {
        ...
        while(true){
            try {
                Thread.sleep(1000);
                System.out.println("Alter Daemon message: "+ Thread.currentThread().getName());
            } catch (InterruptedException e) {
                System.out.println("Alter Daemon message Interrupted 발생");
                break;
            }
        }
